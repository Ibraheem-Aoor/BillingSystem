...
@section('content')
    <a href="#" class="btn btn-sm btn-primary btn-icon rounded-pill cp_link"
        data-link="{{ route('pay.invoice', \Illuminate\Support\Facades\Crypt::encrypt($invoice->id)) }}" data-toggle="tooltip"
        data-original-title="{{ __('Click to copy invoice link') }}">
        <span class="btn-inner--icon text-white"><i class="fas fa-file"></i></span>
                            ...
                            <div class="timeline timeline-one-side" data-timeline-content="axis"
                                ...
                                <div class="timeline-block">
                                    ...
                                    <div class="timeline-content">
                                        ...
                                        @endcan
                                        <small><i class="fas fa-clock mr-1"></i>{{ __('Created on ') }}
                                            {{ \Auth::user()->dateFormat($invoice->issue_date) }}</small>
                                    </div>
                                </div>
                                <div class="timeline-block">
                                    ...
                                    <div class="timeline-content">
                                            ...
                                            <div class="Action">
                                                @can('send invoice')
                                                    <a href="{{ route('invoice.sent', $invoice->id) }}"
                                                        class="send-icon float-right" data-toggle="tooltip"
                                        ...
                                        @if ($invoice->status != 0)
                                            <small><i class="fas fa-clock mr-1"></i>{{ __('Sent on') }}
                                                {{ \Auth::user()->dateFormat($invoice->send_date) }}</small>
                                        @else
                                            @can('send invoice')
                                        ...
                                        @if ($invoice->status != 0)
                                            @can('create payment invoice')
                                                <a href="#" data-url="{{ route('invoice.payment', $invoice->id) }}"
                                                    ...
                                                    class="edit-icon float-right" data-toggle="tooltip"
                                                    data-original-title="{{ __('Add Receipt') }}"><i class="far fa-file"></i></a>
                                            @endcan
                                        @endif
                    ...
                    @if ($invoice->status != 4)
                        <div class="all-button-box mx-2">
                            <a href="{{ route('invoice.payment.reminder', $invoice->id) }}"
                                class="btn btn-xs btn-white btn-icon-only width-auto">{{ __('Receipt Reminder') }}</a>
                        </div>
                    @endif
                    <div class="all-button-box mx-2">
        ...
        <div class="row justify-content-between align-items-center mb-3">
            <div class="col-md-12 d-flex align-items-center justify-content-between justify-content-md-end">
                ...
                @if ($invoice->getDue() > 0 &&
                    ...
                    <div class="all-button-box">
                        <a href="#" class="btn btn-xs btn-white btn-icon-only width-auto" data-toggle="modal"
                            ...
                            <div class="row invoice-title mt-2">
                                ...
                                <div class="col-xs-12 col-sm-12 col-nd-6 col-lg-6 col-12 text-right">
                                    <h3 class="invoice-number">
                                        {{ AUth::user()->invoiceNumberFormat($invoice->invoice_id) }}</h3>
                                </div>
                                <div class="col-12">
                                    <hr>
                                </div>
                            ...
                            <div class="row mt-3">
                                <div class="col">
                                    <small>
                                        ...
                                        @if ($invoice->status == 0)
                                            <span
                                                class="badge badge-pill badge-primary">{{ __(\App\Models\Invoice::$statues[$invoice->status]) }}</span>
                                        @elseif($invoice->status == 1)
                                            <span
                                                class="badge badge-pill badge-warning">{{ __(\App\Models\Invoice::$statues[$invoice->status]) }}</span>
                                        @elseif($invoice->status == 2)
                                            <span
                                                class="badge badge-pill badge-danger">{{ __(\App\Models\Invoice::$statues[$invoice->status]) }}</span>
                                        @elseif($invoice->status == 3)
                                            <span
                                                class="badge badge-pill badge-info">{{ __(\App\Models\Invoice::$statues[$invoice->status]) }}</span>
                                        @elseif($invoice->status == 4)
                                            <span
                                                class="badge badge-pill badge-success">{{ __(\App\Models\Invoice::$statues[$invoice->status]) }}</span>
                                        @endif
                                    </small>
                                </div>
                                    ...
                                    <td>
                                        @can('edit credit note')
                                            <a data-url="{{ route('invoice.edit.credit.note', [$creditNote->invoice, $creditNote->id]) }}"
                                                ...
                                                href="#" class="edit-icon" data-toggle="tooltip"
